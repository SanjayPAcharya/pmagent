...
### - [x] B3 — Web: archived-projects page + DangerZone → Archive (M) *(done 2026-07-07)*
New per-org route `/orgs/:slug/archived` (`ArchivedProjects.tsx`) — archived projects with Restore + Delete permanently (inline two-click confirm + 4s reset, `Loader2`, shared `EmptyState`); "Archived" link in the OrgProjects header. Project Danger Zone now **archives** (recoverable) instead of hard-deleting: `api.archiveProject`, copy → "Archive this project…restore it any time from Archived projects", keeps the typed-name confirm. api client gained `archiveProject`/`restoreProject` + `listProjects(orgId,{archivedOnly})`. i18n `archived.projects*` + `settings.archive*`. **Browser-verified** (throwaway project ZAT): Danger-Zone archive hid it from grid + sidebar → listed on `/archived` → **Restore** returned it to the workspace (EmptyState after) → re-archive → **permanent delete** removed it entirely; ZAT cleaned up. web 32/32 + typecheck + build green. FEATURES updated.

**Phase 3.7.3 COMPLETE** — api 81/81, web 32/32. Archived tickets *and* projects can now be seen and retrieved.
