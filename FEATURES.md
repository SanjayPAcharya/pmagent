...
- Three optional **AI helpers**, powered by a cloud AI service (Amazon Bedrock). Each one only ever produces a **draft you review and edit** — nothing is saved or created until you say so.
  - **Draft with AI** (board quick-add) — jot a few rough notes in the add-card box, click *Draft with AI*, and get a suggested title, description, acceptance criteria, and priority. The preview is **fully editable**: tweak the title or description, switch the priority, and **untick any acceptance-criteria bullets** you don't want — the ticket you create is exactly what you approved. Not quite right? **Regenerate** for another take (it asks first if you've made edits), or **Discard** — your notes stay in the box either way.
  - **Auto-fill from prompt** (ticket panel) — on a ticket's Spec, generate a fuller description, goal, acceptance criteria, and constraints; an optional one-line steer lets you point the AI at what matters. You then get a **side-by-side review**: each field shows your current text vs. the AI's proposal with an accept tick. Empty fields are pre-accepted; anything that would **replace existing text** is flagged and left unticked by default, so nothing is overwritten unless you say so. **Accept all** / **Keep all current** toggle everything at once. Apply lands your choices in the fields (in edit mode) and the normal **Save** stores them — the AI never saves on its own.
  ...
  - **Draft goal with AI** (Sprints) — while editing a sprint's goal, generate a one-sentence goal from the sprint's committed tickets. It types into the field for you to tweak and Save; nothing is saved until you do.
...
- While the AI works you'll see a compact **"thinking" animation** that shimmers through *Thinking → Cooking → Finishing*, and a **Cancel** button (Esc works too) if you change your mind — cancelling never shows an error. When the result arrives it **types itself onto the screen** — drafts and summaries reveal title first then details line by line, and Auto-fill writes into the spec fields one after another, like watching it being written. You don't have to wait for the animation: Create works the moment the result appears, and it's skipped entirely if your system prefers reduced motion.
