...
- **Now (2026-07-02):** 3.1 (search / list view / my-work / relations / blocked badges / bulk) and 3.2 (mention chips, reactions) shipped and **browser-verified end-to-end** on the dev stack. Migration `20260702150624_comment_reactions` applied (dev + test). API suite **45/45**. Fixed: stale Prisma client in the api container broke comments (see Blocked notes); board/list scrollbars now slim + theme-aware.
- **Next:** pick from the 3.x backlog — **3.3 insights/reports** · **3.4 workflow (templates, unblock nudges, CSV)** · **3.5 hygiene (settings pages → monitoring → plan limits → API tokens/webhooks)** — then **Phase 4 (notification channels)** and the **Phase 5 agent block**. When ready to ship: merge `dev` → `main` (images deploy automatically; compose/infra config changes still applied manually via SSH).
...
|---|---|---|---|
| 2026-07-03 | docs | **`FEATURES.md` — plain-language product guide** for end users covering everything shipped (sign-in, orgs/roles/invites, navigation & ⌘K, board, list + CSV, tickets/relationships/templates, comments & reactions, bulk, sprints, automation toggles, personalization) + a "coming next" section. Convention added to `CLAUDE.md`: update it in the same commit whenever a user-facing feature ships. | — |
