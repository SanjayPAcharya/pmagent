...
- **Now:** 🟡 **Phase 2 in progress — 2A + 2B + 2C done (backend complete).** 2C adds: sprint CRUD + start/complete (velocity) + completion counts + add/remove tickets (cross-scope guard); Redis event bus wired into `buildServer` (single subscription → multi-handler dispatch) with `onClose` dispose; WS server `/ws` (hardened handshake: auth-timeout→4001, token verify via shared jose verifier, project-membership gate, project+user rooms, presence, fan-out by projectId/userId); caller-scoped in-app notification service (assignee/creator/watchers/@mentioned − actor → `Notification` + `notification.new`); org invite tokens (CSPRNG, role-cap, single-use, expiry, accept) + shared `WSMessage` type. typecheck/build green, **29 tests pass** (+10: sprints/invites/notification-IDOR/WS handshake+delivery).
- **Next:** **2D — Frontend foundation:** shadcn/ui adoption; routing restructure (public `/invite/:token` vs gated); members endpoint + `api.listMembers`; typed WS client (refresh + reconnect-refetch + echo-dedupe). Then 2E (board + drawer + verify).
- **Blocked:** none. Notes: `corepack` flake — pin `corepack pnpm@9.12.0` (bare `corepack pnpm` grabbed 11.8.0 → `ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING`); `COREPACK_INTEGRITY_KEYS=0` still needed for install. Realtime tests need Redis (host `:6379`); other test files stay hermetic. CI (Postgres+Redis+Playwright) finalized in Phase 3.
...
**Status:** 🟡 in progress — **2A + 2B + 2C done (backend complete); 2D–2E (frontend) next.**
