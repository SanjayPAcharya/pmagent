...
// the generic message. Returns null for a user-initiated cancel (B2) — the
// caller drops it silently instead of showing an error.
export function aiErrorKey(err: unknown): string | null {
  // fetch aborts throw DOMException, which does NOT extend Error — match on
  // the name alone.
